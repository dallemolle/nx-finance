...
- `verify-reconciliation.ts` / `verify-csv-installment-import.ts` / `verify-confirm-estimate.ts` / `verify-invoice-timeline-detail.ts` / `verify-notifications.ts` / `verify-invoice-month-display.ts`: integração contra o banco de dev, criam e limpam seus próprios dados de teste (usuário descartável, cascade delete).
...
| `faturas/invoice-analysis-content.tsx` | Client Component | Uma seção por cartão (empilhadas, sem abas), cada uma com `StackedInvoiceBarChart` + lista mês a mês **já aberta por padrão** (chevron pra colapsar) mostrando os itens (`descricao`, categoria, data, valor, `ProvisionedBadge`) que compõem o total daquele mês |
...
| `credit-card-invoice-dialog.tsx` | Client Component | Importação de fatura de cartão de crédito via CSV, em 2 passos: (1) upload + descrição/vencimento/instituição/meio de pagamento + seletor de `CreditCard` opcional; (2) revisão item a item, com um botão por linha (`Popover`, ícone `Repeat`) pra marcar "isto é uma compra parcelada" (nº da parcela + total), habilitado só se um cartão foi selecionado no passo 1 — **só ao clicar em "Ativar"** (nunca automaticamente) o app tenta reconhecer o padrão de parcela na descrição via `detectInstallmentInDescription()` e pré-preenche os campos; a descrição do item nunca é alterada. Cria cabeçalho `is_invoice_header` + itens; aceita valores negativos (estorno/reembolso, reduzem o total da fatura); sugestão automática de categoria com destaque visual sutil (sem badge de texto) |
...
**Já resolvido nesta branch (histórico, para não reabrir por engano):** parcela futura de compra parcelada aparecendo com rótulo/mês incorreto em `/faturas` e no timeline do dashboard — causa raiz: `getInvoiceTimeline`/`getInvoiceTimelineDetail` e a descrição gerada por `findOrCreateProvisionedHeader` usavam `invoice_month`/`invoice_year` (ciclo de fechamento, deslocado -1 mês do vencimento quando `dueDay<=closingDay`) como se fosse o mês calendário de exibição; corrigido pra usar o mês calendário de `data_vencimento` só na camada de exibição/agrupamento (`invoice_month`/`invoice_year` seguem exatamente como antes pro uso interno — dedup, offset de parcelas). Descrição das parcelas futuras também parava de duplicar um padrão de parcela já embutido no texto do extrato (`stripInstallmentPattern()`), e `/faturas` ganhou janela dinâmica de meses (antes fixa em 6, cortava parcelamentos longos). Reproduzido e coberto por `scripts/verify-invoice-month-display.ts`. 2FA agora é TOTP funcional de ponta a ponta (ver 3.6); `prisma.ts`, `mail.ts`/`mail.js` e o `dashboard/page.tsx` mock foram removidos (código morto); `export-buttons.tsx` exporta CSV/PDF de verdade; `data_pagamento` é preenchido por `payTransaction()`; `@auth/prisma-adapter` e `nodemailer` foram removidos do `package.json`; módulo completo de compras parceladas/despesa prevista/provisionamento de fatura futura implementado do zero nesta branch (ver 3.10) — as branches antigas que tentavam isso (`feature/2026-07-15_lancto_previsao_despesa`, `07-17_prev_desp`, `07-18_prev-desc-new_table`) não foram portadas e podem ser ignoradas/descartadas; timeout de transação interativa (P2028) em `importCreditCardInvoice`/`provisionCardInstallmentPurchase`/`provisionEstimatedExpense` corrigido reduzindo o número de queries dentro da transação (`createMany` + lookups/aprendizado movidos pra fora, ver nota em 3.4) — a primeira tentativa (só paralelizar via `Promise.all`) não foi suficiente, porque uma transação interativa roda numa única conexão e `Promise.all` não paraleliza de verdade no banco. Se esse erro reaparecer, confirmar primeiro via `/api/version` (4.8) se o commit da correção já está de fato em staging antes de investigar mais.
