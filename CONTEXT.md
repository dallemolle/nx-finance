...
- `verify-reconciliation.ts` / `verify-csv-installment-import.ts` / `verify-confirm-estimate.ts` / `verify-invoice-timeline-detail.ts` / `verify-notifications.ts` / `verify-invoice-month-display.ts` / `verify-duplicate-installment-detection.ts`: integração contra o banco de dev, criam e limpam seus próprios dados de teste (usuário descartável, cascade delete).
...
| `faturas/page.tsx` | Server Component | Página dedicada de análise de faturas de cartão (rota `/faturas`): auth + `getCreditCards()`/`getInvoiceTimelineDetail()` em paralelo; sem cartão cadastrado mostra estado vazio (CTA pra `/dashboard/settings`) |
| `faturas/invoice-analysis-content.tsx` | Client Component | Uma seção por cartão (empilhadas, sem abas), cada uma com `StackedInvoiceBarChart` sempre visível + hierarquia de detalhe em 3 níveis, tudo **fechado por padrão**: botão "Ver meses" no cabeçalho do card revela a lista de meses (só resumo confirmado/previsto/total); clicar num mês revela os itens (`descricao`, categoria, data, valor, `ProvisionedBadge`) daquele período; "Expandir todos os meses"/"Recolher todos os meses" alternam todos os meses **daquele cartão** de uma vez (dois `Set<string>` de estado — `expandedCards`/`expandedMonths` — cada um escopado pra não afetar os outros cartões) |
...
| `credit-card-invoice-dialog.tsx` | Client Component | Importação de fatura de cartão de crédito via CSV, em 2 passos: (1) upload + descrição/vencimento/instituição/meio de pagamento + seletor de `CreditCard` opcional; (2) revisão item a item numa grid `table-fixed` (dialog `sm:max-w-[960px]`, coluna Item mais larga, coluna de ações com `sticky right-0` pra nunca sumir atrás do scroll horizontal). Linha cuja descrição parece "parcela 1 de N" e **ainda não foi ativada** ganha destaque sutil em **sky** + um chip inline "Nº/Total · Ativar" (ativa com 1 clique, sem abrir popover) — distinto do índigo, que marca linha **já** ativada (com `Badge` "Nº/Total"). O fluxo manual (`Popover`, ícone `Repeat`, botão "Ativar" + campos de nº/total) continua disponível pra descrições sem padrão reconhecível ou pra revisar antes de ativar — habilitado só se um cartão foi selecionado no passo 1. **Só ao ativar** (clique no chip ou no "Ativar" do popover — nunca automaticamente) o app tenta reconhecer o padrão de parcela via `detectInstallmentInDescription()` e pré-preenche os campos; a descrição do item nunca é alterada. Se a parcela resultante for nº > 1 (ativação, ou editar o campo "nº" manualmente e sair dele), chama `findPossibleDuplicateInstallments()` **no momento da ativação/edição** (não mais só ao confirmar a fatura inteira) e, se achar uma parcela anterior parecida pro mesmo cartão, pede confirmação (`confirm()`) antes de ativar — cancelar não apaga a linha, só desativa o parcelamento dela (vira lançamento normal). Cria cabeçalho `is_invoice_header` + itens; aceita valores negativos (estorno/reembolso, reduzem o total da fatura); sugestão automática de categoria com destaque visual sutil (sem badge de texto) |
...
| `invoice-timeline-chart.tsx` | Client Component | `BarChart` empilhado (confirmado vs. provisionado) dos próximos meses de fatura por cartão; hospeda os gatilhos de `CardInstallmentPurchaseDialog` e `EstimatedExpenseDialog` no cabeçalho; usa `StackedInvoiceBarChart` internamente |
| `stacked-invoice-bar-chart.tsx` | Componente puro | `BarChart` empilhado confirmado/provisionado extraído de `invoice-timeline-chart.tsx` — reaproveitado também nas seções por cartão de `/faturas` (`InvoiceAnalysisContent`) |
...
- `credit_card_id`/`invoice_month`/`invoice_year` só são preenchidos em cabeçalhos de fatura (`is_invoice_header:true`) vinculados a um `CreditCard` — `invoice_month`/`invoice_year` guardam o **mês de referência** (ciclo de fechamento), não o mês do vencimento, indexados junto (`@@index([userId, credit_card_id, invoice_month, invoice_year])`) pra achar "já existe fatura projetada desse cartão+mês?" sem recalcular a regra de vencimento a cada busca. **Uso exclusivamente interno** (dedup em `findOrCreateProvisionedHeader`, offset em `addInvoiceMonths` ao projetar parcelas) — nunca usar esse par pra decidir em qual mês calendário exibir/agrupar um lançamento; pra isso o campo certo é sempre `data_vencimento` (ver nota em 3.10, timeline).
...
3. **Marcar item na importação de CSV** (`credit-card-invoice-dialog.tsx`, Step 2) — ao importar a fatura real, um item pode ser marcado como parcela nº X de Y (suporta começar no meio de um parcelamento, ex. entrar no app já na parcela 3/6); o item importado **é** a parcela real, então só as parcelas restantes são projetadas nos meses seguintes, **com o mesmo valor e a mesma `data_compra` do item importado** (extrato de banco já mostra o valor fixo da parcela — não se divide um total; a data de compra não muda porque a compra aconteceu uma única vez, mesmo aparecendo em N faturas). Ao clicar em "Ativar" pra marcar um item, `detectInstallmentInDescription()` (`dashboard-utils.ts`) tenta reconhecer o padrão de parcela já embutido na descrição do extrato (`"Parcela 1/3"`, `"1-5"`, `"1 de 5"`) e pré-preenche nº/total — só dispara nesse clique manual, nunca sozinho ao carregar o CSV, e nunca edita a descrição do item. Já a descrição gerada pra cada parcela **futura** (novo registro, não o item importado) passa por `stripInstallmentPattern()` antes de anexar `(NN/NN)` — remove um padrão de parcela já embutido no texto do extrato pra não gerar rótulos contraditórios como "Parcela 1/10 (02/10)".
...
**Timeline e comprometimento** (`getInvoiceTimeline`, `InvoiceTimelineChart` + `MonthlyCommitmentCard`): agrega por mês (bucket) os totais confirmado vs. provisionado dos próximos 6 meses, por cartão. O card de comprometimento mostra quanto da renda do mês corrente já está comprometido (parcelas + estimativas), com faixas de cor por limiar (mesmo padrão de `financial-health.tsx`). Os buckets são chaveados pelo mês/ano **calendário de `data_vencimento`** de cada cabeçalho — nunca por `invoice_month`/`invoice_year` (ver nota em 3.2): esses dois só coincidem quando `dueDay > closingDay`; no caso comum (`dueDay <= closingDay`), usar o ciclo como chave de bucket faria uma fatura com vencimento em setembro aparecer rotulada/agrupada como agosto (bug real, corrigido — reproduzido em `scripts/verify-invoice-month-display.ts`).
...
**Página dedicada `/faturas`** (`src/app/faturas/`): o card de timeline no dashboard principal soma confirmado+provisionado por mês entre **todos** os cartões juntos — pra analisar mês a mês *quais despesas especificamente* compõem esse total, agrupado por cartão, existe essa página separada (link no menu, `getInvoiceTimelineDetail()`). Uma seção por cartão (empilhadas, sem abas — dá pra comparar sem clicar), cada uma com o mesmo gráfico de barras (`StackedInvoiceBarChart`, extraído de `InvoiceTimelineChart` pra reuso) e, abaixo, a lista de itens de cada mês **já expandida por padrão**. Sem cartão cadastrado, mostra estado vazio orientando a cadastrar um em Configurações. Diferente do timeline do dashboard (sempre 6 meses fixos, visão rápida), quando `getInvoiceTimelineDetail()` é chamada sem `monthsAhead` explícito ela estende a janela automaticamente até o vencimento da fatura provisionada mais distante do usuário (teto de segurança: 36 meses) — sem isso, um parcelamento de 10x+ tem parcelas que existem no banco mas nunca aparecem aqui.
...
- **TopNav** (`src/components/layout/top-nav.tsx`): barra fixa com backdrop-blur; logo à esquerda linka pra `/` (sem botão "voltar"); gatilho de busca central que abre o `CommandPalette` (Ctrl/Cmd+K); links Dashboard/Relatórios/Faturas/Configurações à direita com active state via `pathname`, ocultos em mobile (`hidden sm:flex`).
...
Na grid de revisão de `credit-card-invoice-dialog.tsx`, índigo já significa "linha marcada como parcelada" — por isso o destaque de "linha candidata a parcela 1/N, ainda não ativada" usa **sky** (`bg-sky-50 border-sky-200 dark:bg-sky-950/20 dark:border-sky-900`) em vez de âmbar (reservado acima) ou de reaproveitar o próprio índigo (confundiria "já ativado" com "ainda não ativado, precisa de ação").
...
- **Proteção contra double-provisioning é só um aviso, não uma trava**: `findPossibleDuplicateInstallments()` (ver `credit-card-actions.ts`) avisa (via `confirm()`) quando um item é marcado direto numa parcela nº > 1 e existe uma parcela anterior parecida pro mesmo cartão — cobre o caso mais comum (reimportação acidental do mesmo CSV/parcelamento). Continua **sem** proteção se o usuário lança uma compra parcelada manualmente (diálogo "Compra Parcelada") e depois importa a fatura real marcando o mesmo item como parcela 1 (não dispara o aviso, que só olha parcela nº > 1) — os dois fluxos ainda geram projeções independentes e não-deduplicadas nesse caso. E mesmo quando o aviso dispara, confirmar sempre cria uma cadeia nova (não tenta mesclar com a existente) — decisão deliberada de escopo, ver commit da feature.
...
**Já resolvido nesta branch (histórico, para não reabrir por engano):** parcela futura de compra parcelada aparecendo com rótulo/mês incorreto em `/faturas` e no timeline do dashboard — causa raiz: `getInvoiceTimeline`/`getInvoiceTimelineDetail` e a descrição gerada por `findOrCreateProvisionedHeader` usavam `invoice_month`/`invoice_year` (ciclo de fechamento, deslocado -1 mês do vencimento quando `dueDay<=closingDay`) como se fosse o mês calendário de exibição; corrigido pra usar o mês calendário de `data_vencimento` só na camada de exibição/agrupamento (`invoice_month`/`invoice_year` seguem exatamente como antes pro uso interno — dedup, offset de parcelas). Descrição das parcelas futuras também parava de duplicar um padrão de parcela já embutido no texto do extrato (`stripInstallmentPattern()`), e `/faturas` ganhou janela dinâmica de meses (antes fixa em 6, cortava parcelamentos longos). Reproduzido e coberto por `scripts/verify-invoice-month-display.ts`. Grid de revisão da importação de fatura (`credit-card-invoice-dialog.tsx`) ganhou coluna Item mais larga + `table-fixed` + coluna de ações `sticky` (não sumia mais atrás do scroll horizontal) e badge de parcelamento; `findPossibleDuplicateInstallments()` avisa (não trava) reimportação acidental de parcela já provisionada (nº > 1 do mesmo cartão/estabelecimento), coberto por `scripts/verify-duplicate-installment-detection.ts` — inicialmente rodava em lote só ao confirmar a fatura inteira, depois movido pra rodar no momento em que a parcela nº > 1 é definida (ativação ou edição do campo), com a checagem em lote removida pra não repetir o aviso. Linha cuja descrição parece parcela 1/N ganhou destaque sky + atalho de ativação de 1 clique (fora do popover), reduzindo a interação de abrir popover + clicar "Ativar" pra um clique só nos casos auto-detectados. `/faturas` (`invoice-analysis-content.tsx`) ganhou hierarquia de 3 níveis (gráfico → lista de meses → despesas do mês) tudo fechado por padrão, com atalho pra expandir/recolher todos os meses de um cartão de uma vez. 2FA agora é TOTP funcional de ponta a ponta (ver 3.6); `prisma.ts`, `mail.ts`/`mail.js` e o `dashboard/page.tsx` mock foram removidos (código morto); `export-buttons.tsx` exporta CSV/PDF de verdade; `data_pagamento` é preenchido por `payTransaction()`; `@auth/prisma-adapter` e `nodemailer` foram removidos do `package.json`; módulo completo de compras parceladas/despesa prevista/provisionamento de fatura futura implementado do zero nesta branch (ver 3.10) — as branches antigas que tentavam isso (`feature/2026-07-15_lancto_previsao_despesa`, `07-17_prev_desp`, `07-18_prev-desc-new_table`) não foram portadas e podem ser ignoradas/descartadas; timeout de transação interativa (P2028) em `importCreditCardInvoice`/`provisionCardInstallmentPurchase`/`provisionEstimatedExpense` corrigido reduzindo o número de queries dentro da transação (`createMany` + lookups/aprendizado movidos pra fora, ver nota em 3.4) — a primeira tentativa (só paralelizar via `Promise.all`) não foi suficiente, porque uma transação interativa roda numa única conexão e `Promise.all` não paraleliza de verdade no banco. Se esse erro reaparecer, confirmar primeiro via `/api/version` (4.8) se o commit da correção já está de fato em staging antes de investigar mais.
