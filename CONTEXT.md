...
| `credit-card-invoice-dialog.tsx` | Client Component | Importação de fatura de cartão de crédito via CSV, em 2 passos: (1) upload + descrição/vencimento/instituição/meio de pagamento + seletor de `CreditCard` opcional; (2) revisão item a item numa grid `table-fixed` (dialog `sm:max-w-[960px]`, coluna Item mais larga, coluna de ações com `sticky right-0` pra nunca sumir atrás do scroll horizontal), com um botão por linha (`Popover`, ícone `Repeat`) pra marcar "isto é uma compra parcelada" (nº da parcela + total), habilitado só se um cartão foi selecionado no passo 1 — **só ao clicar em "Ativar"** (nunca automaticamente) o app tenta reconhecer o padrão de parcela na descrição via `detectInstallmentInDescription()` e pré-preenche os campos; a descrição do item nunca é alterada. Linha parcelada ganha um `Badge` sutil "Nº/Total" em índigo (mesma cor já usada nesta tela pro estado de parcelamento). Ao confirmar, se algum item foi marcado direto numa parcela nº > 1, chama `findPossibleDuplicateInstallments()` e, se achar uma parcela anterior parecida pro mesmo cartão, pede confirmação (`confirm()`) antes de prosseguir — cancelar não apaga a linha, só desmarca o parcelamento dela (vira lançamento normal). Cria cabeçalho `is_invoice_header` + itens; aceita valores negativos (estorno/reembolso, reduzem o total da fatura); sugestão automática de categoria com destaque visual sutil (sem badge de texto) |
